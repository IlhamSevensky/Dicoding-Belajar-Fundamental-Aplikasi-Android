...
class MyWorker(context: Context, workerParams: WorkerParameters) : Worker(context, workerParams) {
    ...
    companion object {
        ...
        const val APP_ID = "ec9026ef3d24af641bcae2eb6505ac64"
        const val EXTRA_CITY = "Semarang"
        const val NOTIFICATION_ID = 1
        const val CHANNEL_ID = "channel_01"
        const val CHANNEL_NAME = "work manager channel"
    }

    private var resultStatus: Result? = null

    override fun doWork(): Result {
        val dataCity = inputData.getString(EXTRA_CITY) // mengambil data yang dikirim dari main activity ke work manager
        val result = getCurrentWeather(dataCity)
        return result
    }

   /*
        WorkManager sudah berjalan di background thread, jadi aman saja menjalankan secara langsung.
    ...
    */
    private fun getCurrentWeather(city: String?): Result {
        ...
        val client = SyncHttpClient() // syncronus, bukan asyncronus. di workmanager harus pake syncronus untuk mendapatkan result success
